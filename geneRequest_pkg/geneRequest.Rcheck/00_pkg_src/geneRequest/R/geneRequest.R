...
geneRequest <- function(geneId, DB = "gene", bySymb = TRUE, verbose = TRUE){
  ...
  if(bySymb){
    ...
    ids <- unlist(.gsearch(paste0(query, "%5Bsymbol%5D%20homo%20sapiens"), DB = DB, kTries = 10))
      ...
      geneSummary <- .gsummary(geneId, DB = DB, kTries = 10)
      out <- .getSummary(geneSummary, cumLen)
...
.gsearch <- function (symbol, DB, kTries){
  
  # symbol : use official symbols. Multiple requests are accepted, e.g. "EGFR, Homo sapiens"
  ...
  URL <- paste(gsrch.stem, gsrch.mode, symbol, sep = "")
  k = 1
