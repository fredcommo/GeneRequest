...
GeneRequest.v6.1 <- function(geneList, DB = "gene", bySymb = TRUE, verbose = TRUE){
...
# require(tcltk2)
  ...
  Total = length(geneList)
    ...
    if(bySymb){
      ...
      ids <- unlist(gsearch(paste(gname, "[symbol] homo sapiens", sep = ""), DB = DB))					#homo sapiens
  		...
  		if(!is.null(ids)){ 
			...
			j = 1
			Id <- ids[j]
			gsum <- unlist(gsummary(paste(Id, "&#91UID&#93", sep = ""), DB = DB))					#homo sapiens
			foundName <- gsum[1]
			Org <- gsum[3]
			status <- gsum[2]
			...
			if(length(ids)>1){
				while ((foundName != gname| Org != "Homo sapiens" | status=="reserved" | verifId != 0 | Chr =="") & j < length(ids)){
					j = j + 1
					Id <- ids[j]
					gsum <- unlist(gsummary(paste(Id, "&#91UID&#93"), DB = DB))			#homo sapiens
					foundName <- gsum[1]
					status <- gsum[2]
					Org <- gsum[3]
					verifId <- as.numeric(gsum[5])
	...
	colnames(gene.annot) <- c("Query", "Symbol", "Alias", "Name", "Org", "Chr", "Cytoband", "Chr.start", "Chr.end","RangeGB.Id", "GeneId")
#	close(pb)
	return(gene.annot)
}
