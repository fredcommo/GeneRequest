...
GeneRequest.v6.2 <- function(geneList, DB = "gene", bySymb = TRUE, verbose = TRUE){

require(XML)
  hg19 <- read.table('/Users/fredcommo/Documents/MyProjects/ArraysInfos/human.chrom.info.hg19.FC.txt',
                     header = TRUE)
  ...
  Total = length(geneList)
    ...
    if(bySymb){
      gname <- geneList[i]
      ids <- unlist(gsearch(paste(gname, "%5Bsymbol%5D%20homo%20sapiens", sep = ""), DB = DB))					#homo sapiens
  		...
  		if(!is.null(ids)){ 
			...
			j = 1
			Id <- ids[j]
			gsum <- unlist(gsummary(Id, DB = DB))					#homo sapiens
			foundName <- gsum[1]
			Org <- gsum[3]
			status <- gsum[2]
      ...
      if(!bySymb){
        ...
        cat(as.character(Id), "found as", gname, "\n")
			...
			Chr <- as.numeric(Chr)
			RangeId <- gsum[20]
			ChrStart <- as.numeric(gsum[21])
			ChrStop <- as.numeric(gsum[22])
			if(length(gsum)!=27){
				RangeId <- gsum[19]
				ChrStart <- as.numeric(gsum[20])
				ChrStop <- as.numeric(gsum[21])
				}
			genomStart <- ChrStart + cumLen[Chr]
			genomStop <- ChrStop + cumLen[Chr]
			}
		gene.annot <- rbind(gene.annot, c(gname, Symb, alias, Name, Org, Chr, Cytoband,
                                      ChrStart, ChrStop, genomStart, genomStop, RangeId, Id))
		}	

	gene.annot <- as.data.frame(gene.annot)
	colnames(gene.annot) <- c("Query", "Symbol", "Alias", "Name", "Org", "Chr", "Cytoband",
                            "ChrStart", "ChrStop", "genomStart", "genomStop", "RangeGB.Id",
                            "entrezgene")
#	close(pb)
	return(gene.annot)
}
