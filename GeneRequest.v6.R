    ...
    if(bySymb){
      ...
      ids <- unlist(gsearch(paste(gname, "[symbol] homo sapiens", sep = ""), database = DB))					#homo sapiens
  		...
  		if(!is.null(ids)){ 
			...
			j = 1
			Id <- ids[j]
			gsum <- unlist(gsummary(paste(Id, "[UID] homo sapiens", sep = ""), database = DB))					#homo sapiens
			foundName <- gsum[1]
			Org <- gsum[3]
			status <- gsum[2]
			...
			if(length(ids)>1){
				while ((foundName != gname| Org != "Homo sapiens" | status=="reserved" | verifId != 0 | Chr =="") & j < length(ids)){
					j = j + 1
					Id <- ids[j]
					gsum <- unlist(gsummary(paste(Id, "[UID] homo sapiens"), database = DB))			#homo sapiens
					foundName <- gsum[1]
					status <- gsum[2]
					Org <- gsum[3]
					verifId <- as.numeric(gsum[5])
...
gsearch <- function (geneSymb, database){
  ...
  if(class(doc) == 'try-error') stop ('Connection temporarily unavailable. Check it manually:\n', URL)
